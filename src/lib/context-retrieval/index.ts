...
export interface ContextRetriever {
  /**
   * Index content for an ember (for vector DB implementations)
   * For simple retriever, this is a no-op since content is already in DB
   */
  indexContent(emberId: string, content: ContextContent[]): Promise<void>;

  /**
   * Retrieve relevant context for a query
   * Returns concatenated context string ready for LLM
   */
  retrieve(emberId: string, query: string, limit?: number): Promise<string>;
}
