...
export async function PATCH(
  ...
  try {
    ...
    if (body.contributorId !== undefined) {
      if (body.contributorId === null) {
        ...
        const ec = await prisma.emberContributor.findFirst({
          where: { id: body.contributorId, emberId: id },
          select: {
            id: true,
            userId: true,
    ...
    const existingTag = await prisma.emberTag.findFirst({
      where: {
        id: tagId,
        emberId: id,
      },
    ...
    const tag = await prisma.emberTag.update({
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.emberTag.deleteMany({
      where: {
        id: tagId,
        emberId: id,
      },
