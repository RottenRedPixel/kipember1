...
export async function POST(
  ...
  try {
    ...
    const { ember: imageRecord, imageTitle: title, confirmedPeople, confirmedLocation, contributorMemories, callSummaries, callHighlights } = context;
    ...
    const existingSnapshot = await prisma.snapshot.findUnique({ where: { emberId: id } });
...
export async function PATCH(
  ...
  try {
    ...
    const existing = await prisma.snapshot.findUnique({ where: { emberId: id } });
    if (!existing) {
